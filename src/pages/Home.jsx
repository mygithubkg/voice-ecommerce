...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-900">
      {/* Hero Section */}
      <section className="w-full bg-gradient-to-r from-indigo-600 to-cyan-600 py-16 mb-10 shadow-lg rounded-b-3xl">
        <div className="w-4/5 max-w-[1600px] mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-8 text-lg">
          <div className="flex-1">
            ...
            <button className="bg-white hover:bg-slate-100 text-indigo-600 font-bold px-8 py-3 rounded-full shadow-lg text-lg transition-all duration-200 hover:scale-105" onClick={() => navigate("/products")}>Browse Products</button>
          </div>
      ...
      <div className="w-4/5 max-w-[1600px] mx-auto py-8 px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {categories.map((cat) => (
            <div
              key={cat.name}
              className="bg-slate-800 rounded-2xl shadow-lg p-6 flex flex-col items-center border border-slate-700 hover:border-indigo-500 hover:shadow-indigo-500/20 hover:-translate-y-1 transition-all duration-200 group cursor-pointer"
              onClick={() => navigate(`/products#${cat.name.toLowerCase()}`)}
            >
              <div className="mb-3 text-indigo-400 text-4xl">
                <span role="img" aria-label={cat.name}>{cat.emoji || "🛒"}</span>
              </div>
              <h3 className="font-bold text-2xl text-white mb-1 tracking-wide">
                {cat.name}
              </h3>
